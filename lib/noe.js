...
class Noe {
    ...
    _registerMany(many = [], regFn) {
        many.forEach((m) => {
            console.log('_registerMany start');
            if (is.object(m)) {
                console.log('_registerMany isObject');
                if (is.array(m)) {
                    console.log('_registerMany isArray');
                    regFn(m[0], m[1], m[2]);
                } else {
                    console.log('_registerMany notArray');
                    const cdeps = m.deps === undefined ? m.ctor : m.deps.concat(m.ctor);
                    regFn(m.name, cdeps);
                }
            } else if (typeof arguments[0] !== 'function') {
                console.log('_registerMany notObject');
                regFn(m);
            }
        });
        return this;
